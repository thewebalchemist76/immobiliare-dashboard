...
export default function MonitorMarket({
  ...
  // Weekly (da agency_runs)
  const [weekly, setWeekly] = useState([]); // [{week,newCount,runCount}]
  const [kpi, setKpi] = useState({ new7: 0, avg4w: 0, runs: 0 });

  // Market by zone (da listings + agency_listings + assignments)
  const [zone, setZone] = useState("");
  const [zones, setZones] = useState([]); // dropdown
  const [zoneRows, setZoneRows] = useState([]); // tabella tipo excel
  ...
  const loadWeekly = async () => {
    ...
    try {
      ...
      const avg4w = last4.reduce((s, x) => s + (x.newCount || 0), 0) / Math.max(1, last4.length);
      const runs = out.reduce((s, x) => s + (x.runCount || 0), 0);
  ...
  const loadZonesAndZoneTable = async () => {
    ...
    try {
      // 1) prendo tutti gli id listing della agency
      ...
      // 2) carico listings (solo raw + first_seen_at + price, niente “raw view” in UI)
      // NB: Supabase ha limite su IN troppo grande: qui assumiamo volume gestibile; se cresce, si chunk-a.
      ...
      // 3) assignments per questi id (serve per OK/Potenziale)
      ...
      const assignMap = {};
      (asg || []).forEach((r) => {
        assignMap[r.listing_id] = r.agent_user_id;
      });
      ...
      // se zona non impostata, scegli la prima
      const z = zone && zset.has(zone) ? zone : zlist[0] || "";
      setZone(z);

      // 5) costruisci “tabella excel” per la zona scelta
      const inZone = z
        ? all.filter((x) => String(x?.raw?.analytics?.macrozone || "").trim() === z)
        : [];

      // raggruppo per advertiserLabel (come Annunci)
      const byAdv = new Map(); // adv -> {ok,pot,ver}
      let ok = 0,
        pot = 0,
        ver = 0;

      for (const x of inZone) {
        const adv = (getAdvertiserLabel ? getAdvertiserLabel(x) : "") || "—";
        const advKey = String(adv);

        const mz = String(x?.raw?.analytics?.macrozone || "").trim();
        const isVer = !mz; // “Da verificare” = macrozone vuota
        const isOk = !!assignMap?.[x.id]; // OK = assegnato
        const isPot = !isOk && !isVer; // Potenziale = non assegnato (ma ha zona)

        const cur = byAdv.get(advKey) || { adv: advKey, ok: 0, pot: 0, ver: 0, total: 0 };
        if (isVer) {
          cur.ver += 1;
          ver += 1;
        } else if (isOk) {
      ...
      // righe “excel”: advertiser + counts + %
      const rowsOut = advCounts.map((r) => ({
        adv: r.adv,
        ok: r.ok,
        pot: r.pot,
        ver: r.ver,
  ...
  const exportZoneCSV = () => {
    ...
    downloadCSV(`monitor_zone_${(zone || "all").replace(/\s+/g, "_")}.csv`, [header, ...body]);
  };
  ...
  // reload zone table when zone changes (senza rifare weekly)
  useEffect(() => {
    if (!supabase || !agencyId || !isTL) return;
  ...
  const weeklyMax = useMemo(() => {
    return Math.max(0, ...(weekly || []).map((x) => Number(x.newCount || 0)));
  }, [weekly]);
    ...
    <div className="card">
      ...
      <div style={{ marginTop: 18 }}>
        ...
        <div className="muted" style={{ marginTop: 8, fontWeight: 600 }}>
          Nota: dati da <code>agency_runs.new_listings_count</code> (nessun raw).
        </div>
      </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <select
              ...
              style={{ width: 260, maxWidth: 260, minWidth: 260, padding: "10px 12px", borderRadius: 12 }}
            >
              {(zones || []).map((z) => (
